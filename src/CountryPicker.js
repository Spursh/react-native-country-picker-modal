    ...
    return (
      <View>
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <View style={{width: (Dimensions.get('window').width)*(0.43), height: (Dimensions.get('window').height)/17
            , flexDirection: 'row', borderBottomColor:'#e0e0e0', borderBottomWidth:1, alignItems:'center'}}>

          {this.props.children ? (
            this.props.children
          ) : (
            <View
              style={[styles.touchFlag, { marginTop: 0 }]}
            >
              {CountryPicker.renderFlag(this.props.cca2)}
              {this.props.cca2 ? (CountryPicker.renderFlag(this.props.cca2)) :
                (CountryPicker.renderFlag('US'))}
            </View>
          )}

            <Text style={{alignSelf:'center', color:'#000', width: (Dimensions.get('window').width)*(0.28)}} numberOfLines={1}>
              {this.props.cca2 ? (this.getCountryName(countries[this.props.cca2])) :
              (this.getCountryName(countries['US']))}</Text>
          </View>
        </TouchableOpacity>
        <Modal
